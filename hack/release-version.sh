...
GITLAB_REPO='gitlab.daocloud.cn/ndx/engineering/application/amamba-jenkins.git'
...
if [ "${PRE_VERSION}" = "$(get_version amamba_jenkins)" ]; then
    echo PRE_VERSION should not same as amamba-jenkins in versions.json >>/dev/stderr
    exit 1
fi

if [ "${NEXT_VERSION}" = "$(get_version amamba_jenkins)" ]; then
    echo NEXT_VERSION should not same as amamba-jenkins in versions.json >>/dev/stderr
    exit 1
fi
...
CUR_VERSION=$(get_version amamba_jenkins)
...
f=$(mktemp)
jq ".amamba_jenkins = \"${NEXT_VERSION}\"" ${CUR_DIR}/../versions.json >$f && cat $f >${CUR_DIR}/../versions.json
rm $f
...
if ! git config user.name; then
    ...
    git config user.email "amamba-auto-release@daocloud.io"
fi
